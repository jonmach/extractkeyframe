...
echo "keyframe Images will be of resolution: $RESOLUTION"
#ffmpeg -i $FNAME -vf select='eq(pict_type\,I)' -vsync 2 -q 1 -s $RESOLUTION -f image2 $DIR/${IMGDIR}/thumbnails-%02d.jpg
ffmpeg -i $FNAME -f select='eq(pict_type\,I)' -vsync 2 -q 1 -s $RESOLUTION -f image2 $DIR/${IMGDIR}/thumbnails-%02d.jpg
